 ...
 * Beaker
 */
angular.module('labDirectives').directive('beaker', ['$window', '$interval', '$timeout', 'liquidService', 'explodeService',
    ...
    function ($window, $interval, $timeout, LiquidService, ExplodeService, SteamService, BubbleService, global, SolidService) {
        return {
            restrict: 'AE',
            templateUrl: 'templates/tools/containers/beaker.html',
            scope: {
                uiData: '='
            },
            link: function (scope, element, attrs) {
                ...
                var init = function () {
                    ...
                    element.find('.beaker').css('background-image', 'url(' + scope.uiData.images.backImg + ')');

                    if( /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) )                        
                        element.find('.beaker').find('.beaker-canvas').css('background-image', 'url(' + scope.uiData.images.frontImg + ')');
                    ...
                    liquid = LiquidService.factory({
                        canvas: element.find('.beaker-canvas')[0],
                        canvassolid: element.find('.beaker-canvas-solid')[0],
                        ...
                        width: 335,
                        height: 384,
                        centerX: {top: 182, bottom: 182},
                        centerY: {top: 56, bottom: 316},
                        radiusX: {top: 138, bottom: 120},
                        radiusY: {top: 42, bottom: 63}
                    });

                    solid = SolidService.factory({
                        canvas: element.find('.beaker-canvas-solid')[0],
                        canvas2: element.find('.beaker-canvas-solid2')[0],
                        pourSound: element.find('.pour-sound')[0],
                        volume: scope.uiData.volume,
                        maxAmount: scope.uiData.maxAmount,
                        width: 335,
                ...
                scope.pourLiquid = function(volume) {
                    ...
                    element.find('.beaker').find('.beaker-canvas').removeAttr('style');
                };
                ...
                scope.$on('API:explode', function (evt) {
                    ...
                    explode.play();
                    element.find('.beaker').css('background-image', 'none');
                    element.find('.beaker-canvas').remove();
                    $window.requestTimeout(function() {
                ...
                scope.$on('drag-enter', function () {
                    element.find('.beaker').css('background-image', 'url(' + scope.uiData.images.highlight + ')');
                });
