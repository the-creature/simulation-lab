 ...
 * Evaporating Dish
 */
angular.module('labDirectives').directive('evaporatingdish', ['$window', '$interval', '$timeout', 'liquidService',
    ...
    function ($window, $interval, $timeout, LiquidService, SteamService, BubbleService, global, SolidService) {
        return {
            restrict: 'AE',
            templateUrl: 'templates/tools/containers/evaporatingdish.html',
            scope: {
                uiData: '='
            },
            link: function (scope, element, attrs) {
                ...
                var init = function () {
                    ...
                    element.find('.evaporatingdish').css('background-image', 'url(' + scope.uiData.images.backImg + ')');
                    
                    if( /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) )                    
                        element.find('.evaporatingdish').find('.evaporatingdish-canvas').css('background-image', 'url(' + scope.uiData.images.frontImg + ')');

                    liquid = LiquidService.factory({
                        canvas: element.find('.evaporatingdish-canvas')[0],
                        canvassolid: element.find('.evaporatingdish-canvas-solid')[0],
                        ...
                        width: 169,
                        height: 90,
                        centerX: {top: 88, bottom: 88},
                        centerY: {top: 15, bottom: 78},
                        radiusX: {top: 96, bottom: 48},
                        radiusY: {top: 25, bottom: 9}
                    });

                    solid = SolidService.factory({
                        canvas: element.find('.evaporatingdish-canvas-solid')[0],
                        canvas2: element.find('.evaporatingdish-canvas-solid2')[0],
                        pourSound: element.find('.pour-sound')[0],
                        volume: scope.uiData.volume,
                        maxAmount: scope.uiData.maxAmount,
                        width: 169,
                ...
                scope.pourLiquid = function(volume) {
                    ...
                    element.find('.evaporatingdish').find('.evaporatingdish-canvas').removeAttr('style');
                };
